  ...
  persist(
    (set) => {
      ...
      const handleAppStateChange = (nextAppState: string) => {
        if (nextAppState === 'active') {
          // App came to foreground, setup auth listener again
          setupAuthListener();
        } else if (nextAppState === 'inactive' || nextAppState === 'background') {
          // App went to background, unsubscribe to prevent memory leaks
          if (authUnsubscribe) {
            authUnsubscribe();
      ...
      return {
        ...
        signInWithGoogle: async () => {
          try {
            ...
            set({ user: userCredential.user, loading: false, keepSignedIn: true });
            return true;
          } catch (error: unknown) {
    ...
    },
    {
      ...
      partialize: (state) => ({ 
        ...
        // We don't store the user object in AsyncStorage as Firebase handles that
        // Just store the preference for keeping signed in
      }),
    }
  )
);
